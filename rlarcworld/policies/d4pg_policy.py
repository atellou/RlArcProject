...
class D4PGPolicy(DDPGPolicy):
    ...
    def _init_learn(self) -> None:
        ...
        if self._cfg.learn.noise:
            self._target_model = model_wrap(
                ...
                noise_type="gauss",
                noise_kwargs={"mu": 0.0, "sigma": self._cfg.learn.noise_sigma},
    ...
    def _forward_learn(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # target q value.
        with torch.no_grad():
            next_action = self._target_model.forward(next_obs, mode="compute_actor")[
                "action"
            ]
            next_data = {"obs": next_obs, "action": next_action}
            target_q_dist = self._target_model.forward(
                next_data, mode="compute_critic"
            )["distribution"]
        ...
        if (self._forward_learn_cnt + 1) % self._actor_update_freq == 0:
            ...
            actor_loss = -self._learn_model.forward(actor_data, mode="compute_critic")[
                "q_value"
            ].mean()
